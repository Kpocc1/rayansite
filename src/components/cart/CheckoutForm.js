...
const CheckoutForm = ({ customer, data }) => {
	...
	useEffect(() => {
		// Если доставки нет → переключаемся на самовывоз
		if (
			!deliveryEnabled &&
			pickupEnabled &&
		...
		// Если самовывоза нет → переключаемся на доставку
		if (
			!pickupEnabled &&
			deliveryEnabled &&
			...
			<div className={`white ${isMobile ? 'p-20' : 'p-30'}`}>
				...
				{!deliveryEnabled && (
					<div style={{ marginBottom: 16 }}>
						<Tag color='red'>Курьерская доставка временно недоступна.</Tag>
					</div>
				)}

				{!pickupEnabled && (
					<div style={{ marginBottom: 16 }}>
						<Tag color='red'>Самовывоз временно недоступен.</Tag>
					</div>
				)}
				...
				>
					...
					>
						...
						>
							<TruckOutlined
								style={{ fontSize: 24, verticalAlign: 'top', marginTop: 20 }}
							/>
							<span
								style={{
									fontSize: 16,
							...
							>
								{SHIPPINGS[0].label}
								<br />
								<Tag color={deliveryEnabled ? 'green' : 'red'}>
									{deliveryEnabled
										? costAddress
											? `стоимость: ${formatCurrency(costAddress)}`
											: 'укажите адрес'
										: 'недоступна'}
								</Tag>
							</span>
						...
						>
							...
							>
								{SHIPPINGS[1].label}
								<br />
								<Tag color={pickupEnabled ? 'green' : 'red'}>
									{pickupEnabled ? 'бесплатно' : 'недоступен'}
								</Tag>
							</span>
						</Radio.Button>
					</Radio.Group>
				...
				<Flex justify='space-between' align='flex-end'>
					...
					<div>
						<Typography.Title level={4} style={{ marginTop: 0 }}>
							{formatCurrency(
								subTotal,
								currentShippingMethod !== 'pickup.pickup' ? costAddress : 0
							)}
						</Typography.Title>
